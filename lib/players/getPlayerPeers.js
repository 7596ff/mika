...
/**
 * Returns an array of peers whom the specified player has played with.
 ...
 */
function getPlayerPeers(account_id, options) {
    let query = options ? `?${queryString(options)}` : ``
    return new Promise((resolve, reject) => {
        if (!account_id) reject('Please supply an account id!')
        request(`${BaseURL}/players/${account_id}/peers${query}`, (err, response, body) => {
