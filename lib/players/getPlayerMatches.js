...
/**
 * Returns an array of matches for a specified player id.
 ...
 */
function getPlayerMatches(account_id, options) {
    let query = options ? `?${queryString(options)}` : ``
    return new Promise((resolve, reject) => {
        if (!account_id) reject('Please supply an account id!')
        request(`${BaseURL}/players/${account_id}/matches${query}`, (err, response, body) => {
